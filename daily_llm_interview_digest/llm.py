...
## 4. 今日面试专项训练
必须包含 2 个训练任务，专注大模型实习面试，不要包含算法题：
...
### 4.2 项目表达任务
给出一个能在 1-2 小时内完成的小任务；任务必须服务于 RAG / Agent / LoRA / 模型评测 / 模型部署；明确：
- 任务目标
- 操作步骤
- 验收标准
- 面试时如何讲成项目亮点
...
### 4.2 项目表达任务
- 任务目标：给 RAG 项目加一个最小评测闭环。
- 操作步骤：整理 20 条 query，每条包含标准答案和支持文档片段；跑当前 RAG，记录 top-5 是否命中证据、答案是否引用证据、是否幻觉；把失败 case 分成召回失败、重排失败、生成失败三类。
- 验收标准：能输出一张 bad case 表，并在 README 中写出 3 条具体优化结论。
- 面试时如何讲成项目亮点：可以说“我不是只做了一个能回答的 RAG demo，还做了最小评测闭环，用 recall@5 和 faithfulness 定位问题，并基于 bad case 迭代切分、重排和 prompt。”
