...
3. 真题精讲筛选 3-5 道高价值问题。每道题必须包含：
   ...
   ### 项目联系
   ### 记忆关键词
4. 技术博客精读筛选 2-3 篇。每篇必须包含标题、链接、核心内容总结、对面试有什么用、
   可以转化成的面试题、对应参考答案。
...
### 项目联系
可以讲自己在 RAG 项目里做过 trace 面板：记录 query、召回文档、rerank 分数、引用片段和最终回答，用这些日志定位是召回问题还是生成问题。

### 记忆关键词
召回、重排、chunk、faithfulness、trace
...
### 项目联系
可以把 LoRA 用在“客服问答风格适配”或“领域术语回答”项目中，说明如何准备 SFT 数据、训练 adapter、用验证集比较基础模型和 LoRA 模型。

### 记忆关键词
低秩增量、冻结权重、adapter、rank、合并推理
