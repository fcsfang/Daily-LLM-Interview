...
- 今天至少 2 道真题和 1 个专项理论追问题要明显贴合今日主题。
...
### 题目 X：问题标题
**来源依据：**
直接列出本题使用到的资料标题和链接，例如：
- 资料标题 - https://example.com - 来源类型 - 可信度说明
如果是根据资料延伸出的经典面试题，写“根据上述资料延伸，需进一步验证”。不要只写编号式引用。
...
- 文章标题
- 链接
- 来源依据：直接写资料标题、链接和可信度说明；不要只写编号式引用
...
## 4. 今日面试专项训练
只包含 1 个训练任务，专注大模型理论与面试追问，不要包含算法题或项目任务：

### 4.1 大模型理论追问题
给出一个今日主题相关的理论追问，必须包含：
- 问题
- 面试口头版参考答案
- 深入理解版参考答案
- 2 个可能追问与简短回答
...
引用要求：
1. 参考资料必须在使用处直接写清楚标题和链接，不要在文末统一罗列。
2. 不要输出读者无法直接点击的编号式引用。
3. 技术博客精读中的“链接”必须是以 http:// 或 https:// 开头的真实 URL；如果无法确认真实 URL，写“链接：需进一步验证”，不要写“某编号资料中引用的链接”这类不可点击描述。
4. 来源类型只能写：官方文档 / 论文 / 技术博客 / 岗位 JD / 面经或社媒 / 其他。
5. 对个人博客、论坛、LinkedIn、面经搬运等来源，可信度说明中必须写“需进一步验证”。
...
### 4.1 大模型理论追问题
- 问题：为什么 decoder-only 模型适合生成任务？
- 面试口头版参考答案：decoder-only 模型用因果注意力从左到右预测下一个 token，训练目标和推理时逐 token 生成是一致的，所以天然适合对话、续写和代码生成。工程上它也方便做 KV cache，能减少长对话推理中的重复计算。
- 深入理解版参考答案：decoder-only 架构通过 causal mask 限制当前位置只能看历史 token，因此建模的是 P(x_t | x_<t)。这种自回归目标和生成任务完全一致，模型每一步输出一个 token，再把它拼回上下文继续生成。相比 encoder-only，它不需要额外改造成生成器；相比 encoder-decoder，它结构统一，规模化训练和推理服务更直接。局限是长上下文计算成本高，理解类任务通常需要通过 prompt 转成生成式回答。
